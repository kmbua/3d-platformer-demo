...
@export var wall_jump_impulse := 8.0
...
@export_range(0.0, 1.0) var controller_sensitivity := 0.5
@export var camera_smoothing := 15
@export var tilt_upper_limit := 60
@export var tilt_lower_limit := -22.5
...
@onready var _wall_dust_particles: GPUParticles3D = %WallDustParticles
...
func _ready() -> void:
	...
	Events.flag_reached.connect(func on_flag_reached() -> void:
		...
		_wall_dust_particles.emitting = false
	)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if player_is_using_mouse:
		_target_rotation.x += -event.relative.x * mouse_sensitivity
		_target_rotation.y += event.relative.y * mouse_sensitivity
		_target_rotation.y = clamp(_target_rotation.y, tilt_lower_limit, tilt_upper_limit)
		
	if not player_is_using_mouse:
		# Get the right stick input for rotation (controller input)
		var controller_x = Input.get_action_strength("look_right") - Input.get_action_strength("look_left")
		var controller_y = Input.get_action_strength("look_down") - Input.get_action_strength("look_up")
		
		# Apply controller input to target rotation with sensitivity
		_target_rotation.x += controller_x * controller_sensitivity
		_target_rotation.y -= controller_y * controller_sensitivity
		_target_rotation.y = clamp(_target_rotation.y, tilt_lower_limit, tilt_upper_limit)
	

func _physics_process(delta: float) -> void:
	_camera_pivot.rotation_degrees.x = lerp(_camera_pivot.rotation_degrees.x, _target_rotation.y, camera_smoothing * delta)
	_camera_pivot.rotation_degrees.y = lerp(_camera_pivot.rotation_degrees.y, _target_rotation.x, camera_smoothing * delta)
	...
	var is_just_wall_jumping := Input.is_action_just_pressed("jump") and is_on_wall_only()
	if is_on_wall_only():
		_skin.wall_slide()
		velocity.y = -5
	if is_just_jumping:
		...
		_skin.jump()
		_jump_sound.play()
	elif not is_on_floor() and not is_on_wall_only() and velocity.y < 0:
		_skin.fall()
	elif is_on_floor():
		if ground_speed > 0.0:
			_skin.move()
	...
	_wall_dust_particles.emitting = !is_on_floor() && is_on_wall_only()
